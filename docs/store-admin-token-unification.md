...
## 2026-05-12 관리자 하위 페이지 확장 정리
- `/admin/**`, `/admin/dashboard/**`의 파랑 CTA, 보라 그라데이션, 큰 카드 radius를 공통 운영툴 톤으로 낮췄다.
- `layout.module.css`에 관리자 전체 보정 레이어를 추가하고, 강한 장식이 남던 대시보드/시드/상품 수정/이벤트 폼/모달 컴포넌트는 별도 override로 보강했다.
- 관리자 기능 로직, 권한 판정, 저장 API는 변경하지 않았다.
