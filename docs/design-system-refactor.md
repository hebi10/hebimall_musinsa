...
- 2026-05-14: 신규 채팅 상태에서 모바일 하단 입력 영역이 잘리지 않도록 상담창을 세로 flex 레이아웃으로 바꾸고, 헤더/빠른 버튼/입력 영역은 고정한 채 메시지 영역만 남은 높이에 맞춰 줄어들도록 조정했다.
- 2026-05-14: 상담 메시지 하단이 입력 영역에 가려지지 않도록 상담창을 grid rows 구조로 바꾸고, 메시지 영역의 고정 높이를 제거해 헤더/버튼/입력 영역을 제외한 남은 공간만 스크롤하게 했다.
...
- 브랜드명 `STYNA`는 유지하되, 메인/이벤트 배너의 영문 장식 메타 문구를 한국어 정보형 문구로 바꿨다.
- 이벤트 모바일에서는 상담 플로팅 버튼을 숨기고 카드 높이/오버레이를 낮춰 첫 화면의 CTA 가림과 검은 포스터 반복감을 줄였다.
- React Query Devtools와 Next dev indicator가 로컬 포트폴리오 확인 화면에 보이지 않도록 설정했다.

## 2026-06-05 checkout 요약 박스 반응형 보정
- `/orders/checkout`의 결제 요약 박스가 내부 padding 없이 테두리에 붙어 보이던 문제를 `.summaryContent` 래퍼 적용으로 바로잡았다.
- 주문 입력 영역은 섹션 카드 단위로 묶고, 모바일에서는 sticky 요약을 끄고 버튼/금액 행/포인트 입력 컨트롤의 줄바꿈 여유를 확보했다.
- 배송지/결제수단 선택 상태와 섹션 타이틀, 상품 행, 약관 체크 영역을 같은 낮은 반경·얇은 보더 톤으로 맞춰 checkout 전체 밀도를 정리했다.

## 2026-06-11 메인 슬라이드 배너 보정
- `MainBanner`의 3개 기존 배너 이미지는 유지하고, 카피를 세일/기능명 중심에서 STYNA 에디토리얼 룩북 톤으로 정리했다.
- 각 슬라이드에 `tone` 값을 추가해 배너별 포인트 색과 낮은 글로우만 다르게 적용했다.
- 보라색 오버레이를 중립 블랙/아이보리 계열 그라데이션으로 바꾸고, 좌측 세로 룰과 타이포 대비를 추가해 텍스트 가독성을 보정했다.
- 데스크톱 1280px, 모바일 390px 브라우저 렌더링에서 제목/버튼/메타 텍스트가 겹치지 않는 것을 확인했다.

## 2026-06-22 UI/mock/운영 흔적 정리
- `MainBanner`의 영문 editorial 메타와 추상적인 배너 카피를 할인, 신상품, 베스트 상품 중심의 한국어 정보형 문구로 교체했다.
- `/orders/delivery`의 mock 배송 데이터와 테스트 주문/송장번호 안내를 제거하고, 실제 연결 전 상태로 마이페이지 주문내역 확인을 유도하도록 정리했다.
- 고객센터 공지의 가짜 브랜드 입점, 모바일 앱 업데이트 공지를 상품 업데이트와 주문내역/배송조회 안내로 교체했다.
- 관리자 사용자 쿠폰 화면은 가짜 지급 성공/최근 지급 이력을 제거하고 기능 준비 상태로 표시한다.
- 관리자 리뷰 화면은 `mockReviews` 직접 사용을 제거하고 Firestore 리뷰 조회 기반의 로딩/빈 상태로 전환했다. 서버 검증 API가 없는 삭제/숨김 액션은 성공 처리하지 않는다.

## 2026-06-29 UI 중복 정리

- 메인 배너의 `swiper` 의존성을 제거하고 React state + CSS fade 전환으로 대체했다.
- 구형 상품 Add/Edit 모달과 구형 mock 관리자 대시보드를 삭제하고, 상품 추가는 기존 `EditProductForm`을 재사용한다.
- 관리자 사용자/주문 페이지의 자체 헤더/Nav/logout UI를 제거하고 상위 관리자 레이아웃으로 통일했다.

## 2026-06-30 이벤트 페이지 타이포 보정
- `/events` 목록과 `/events/[eventId]` 상세의 큰 제목, 상세 섹션 제목에만 `--font-heading`을 다시 적용했다.
- 본문, 필터, CTA, 카드 메타는 기존 `Pretendard` 기본 폰트를 유지해 쇼핑몰 UI 가독성을 보존했다.
- 한글 제목이 과하게 눌려 보이지 않도록 이벤트 제목의 `font-weight: 800`과 음수 자간을 줄였다.
