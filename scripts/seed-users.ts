...
const seedPointHistory = async () => {
  ...
  try {
    for (const userPointData of mockPointHistory) {
      const batch = writeBatch(db);
      const pointHistoryRef = collection(db, 'users', userPointData.userId, 'pointHistory');

      userPointData.history.forEach((point) => {
        const pointDoc = doc(pointHistoryRef, point.id);
...
export { seedUsers, seedPointHistory, seedUserData };
