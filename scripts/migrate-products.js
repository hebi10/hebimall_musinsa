...
class ProductMigrationService {
  ...
  static async validateMigration() {
    ...
    try {
      // 전체 상품 수 확인
      const originalProductsSnapshot = await db.collection('products').get();
      ...
      for (const productDoc of originalProductsSnapshot.docs) {
        ...
        // 카테고리별 컬렉션에서 확인
        const categoryProductDoc = await db
          .collection('categories')
          .doc(categorySlug)
          .collection('products')
          .doc(productId)
  ...
  static async showStructureExample() {
    ...
    try {
      ...
      for (const categoryDoc of categoriesSnapshot.docs.slice(0, 3)) { // 처음 3개만
...
async function main() {
  ...
  switch (command) {
    ...
    default:
      console.log('사용법:');
      console.log('  node migrate-products.js [analyze|migrate|validate|example|rollback]');
  }
