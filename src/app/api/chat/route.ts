...
• 주의사항: 쇼핑몰에 벗어나는 답변 절대 금지
• 해당 사이트는 실제 사이트가 아닌 박도영의 포트폴리오 사이트 임을 인지해 주세요.
• 고객센터 전화: 010-4789-7410 (평일 10시-19시)
• 이메일 문의: sevim0104@naver.com
...
구체적인 질문을 해주시면 더 자세히 안내해드리겠습니다!`;
  ...
  // 키워드 기반 응답
  if (lowerMessage.includes('주문') || lowerMessage.includes('배송')) {
    return `🤖 AI 상담원: 주문/배송 관련 문의를 도와드리겠습니다.

📦 **주문 현황 확인**
- 마이페이지 > 주문내역에서 실시간 확인 가능
- 주문번호로 배송 추적 가능
...
더 구체적인 주문번호나 배송지 정보가 있으시면 알려주세요!`;
  }

  if (lowerMessage.includes('반품') || lowerMessage.includes('교환')) {
    return `🤖 AI 상담원: 반품/교환 절차를 안내해드리겠습니다.
...
어떤 상품의 반품/교환을 원하시나요?`;
  ...
  // 일반적인 AI 상담원 응답
  return `🤖 AI 상담원: "${message}"에 대한 문의를 잘 받았습니다.

개발 환경에서 AI 상담원 기능을 시뮬레이션하고 있습니다.
프로덕션에서는 더욱 정확하고 개인화된 답변을 제공할 수 있습니다.

구체적으로 어떤 도움이 필요하신지 말씀해 주시면,
더 자세한 안내를 해드리겠습니다! 😊

예시: "주문 취소하고 싶어요", "사이즈 교환 가능한가요?" 등`;
}
